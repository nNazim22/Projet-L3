...
$Db = new Database("setram");
$Db->connexion_to_server();
$Db->create_db();
$conn = $Db->connect_to_db();  // Assign the connection to $conn


$Admin = new admin(null, "nazim", "nazim@gmail.com", "nazim123", "Admin", $Db->connexion);
$Admin->create_table();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $stmt = $conn->prepare("SELECT Agent_ID, FirstName, LastName, Email,type FROM Agents WHERE Email = :email AND password = :password");
  $stmt->execute(['email' => $Email, 'password' => $password]);

  $Agent = $stmt->fetch(PDO::FETCH_ASSOC);

  $stmt2 = $conn->prepare("SELECT id, Nom,  Email,Type FROM administrareurs WHERE Email = :email AND password = :password");
  $stmt2->execute(['email' => $Email, 'password' => $password]);

  $Admin = $stmt2->fetch(PDO::FETCH_ASSOC);

  $stmt3 = $conn->prepare("SELECT ClientID, Nom,Prenom,  email, genre,date_de_naissance ,type,numero_tlphn FROM users WHERE email = :email AND Password = :password");
  $stmt3->execute(['email' => $Email, 'password' => $password]);
    ...
    <div class="Log_style">
      <div class="LoginFromDiv_v2">
        <form method="post" action="Login.php">
          ...
          <div>
            <a class="Oublier" href="forgot_password.php">Mot de pass Oublier</a>
          </div>
        </form>
